...
@implementation UploadTitlesPictureRequest
- (void)request:(ParamsBlock)paramsBlock result:(RequestResultHandler)resultHandler {
    if (!paramsBlock(self)) {
        return;
    }
    [self.params setObject:self.fileName forKey:@"filename"];
    [[UploadImageManager sharedInstance] POST:UPLOAD_TITLES_IMAGE parameters:self.params  constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
